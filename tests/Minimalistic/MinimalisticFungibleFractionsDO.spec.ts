...
export default async function suite(): Promise<void> {
    describe("MinimalisticFungibleFractionsDO", () => {
        ...
        before(async function () {
            ...
            IFungibleFractionsOperations = new ethers.Interface(factories.contracts.interfaces.IFungibleFractionsOperations__factory.abi);
        ...
        it("revert: only DataIndex can call write function", async function () {
            ...
            await expect(MinimalisticFungibleFractionsDO.connect(user1).getFunction("write")(dp, unknownSelector, "0x")).to.be.revertedWithCustomError(
        ...
        it("revert: InsufficientBalance in transferFrom", async function () {
            ...
            const transferFromArg = ethers.AbiCoder.defaultAbiCoder().encode(
                ["address", "address", "uint256", "uint256"],
                [user2.address, user1.address, id, amount]
            );
        ...
        it("revert: InsufficientBalance in batch transferFrom", async function () {
            ...
            const transferFromArg = ethers.AbiCoder.defaultAbiCoder().encode(
                ["address", "address", "uint256[]", "uint256[]"],
                [user2.address, user1.address, [id1, id2], [amount1, amount2]]
            );
        ...
        it("balanceOf should return 0 for unknown account", async function () {
            ...
            const balanceOf = await DataIndex.connect(user1).getFunction("read")(
        ...
        it("should mint fractions and update balanceOf and totalSupply", async function () {
            ...
            const totalSupply = await DataIndex.connect(user1).getFunction("read")(
                await MinimalisticFungibleFractionsDO.getAddress(),
                dp,
                totalSupplySelector,
                totalSupplyArg
        ...
        it("should transfer fractions and update balanceOf", async function () {
            ...
            const transferArg = ethers.AbiCoder.defaultAbiCoder().encode(
                ["address", "address", "uint256", "uint256"],
                [user2.address, user1.address, id, amountTransfer]
            );
            ...
            const balanceOf2 = await DataIndex.connect(user1).getFunction("read")(
                await MinimalisticFungibleFractionsDO.getAddress(),
                dp,
                balanceOfSelector,
                balanceOfArg2
        ...
        it("balance of batch accounts should be updated after batch transfer", async function () {
            ...
            const transferArg = ethers.AbiCoder.defaultAbiCoder().encode(
                ["address", "address", "uint256[]", "uint256[]"],
                [user2.address, user1.address, [id1, id2], [amountTransfer1, amountTransfer2]]
            );

            await DataIndex.connect(user1).getFunction("write")(await MinimalisticFungibleFractionsDO.getAddress(), dp, transferSelector, transferArg);

            const balanceOfArg = ethers.AbiCoder.defaultAbiCoder().encode(
                ["address[]", "uint256[]"],
                [
                    [user2.address, user2.address, user1.address, user1.address],
                    [id1, id2, id1, id2],
                ]
            );

            const balanceOfEncoded = await DataIndex.connect(user1).getFunction("read")(
                await MinimalisticFungibleFractionsDO.getAddress(),
                dp,
                balanceOfSelector,
                balanceOfArg
        ...
        it("exists should return true after mint", async function () {
            ...
            const existsBeforeEncode = await DataIndex.connect(user1).getFunction("read")(
                await MinimalisticFungibleFractionsDO.getAddress(),
                dp,
                existsSelector,
                existsArg
            ...
            const existsAfterEncoded = await DataIndex.connect(user1).getFunction("read")(
                await MinimalisticFungibleFractionsDO.getAddress(),
                dp,
                existsSelector,
                existsArg
